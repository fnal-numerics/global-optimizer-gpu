...
public:
    ...
    __host__ __device__ DualNumber(double real = 0.0, double dual = 0.0) :
real(real), dual(dual) {}
...
denom);
    ...
    __host__ __device__ friend DualNumber operator-(double lhs, const
DualNumber& rhs) { return DualNumber(lhs - rhs.real, -rhs.dual);
    }

    // operator for double * DualNumber
    __host__ __device__ friend DualNumber operator*(double lhs, const
DualNumber& rhs) { return DualNumber(lhs * rhs.real, lhs * rhs.dual);
    }
};
...
__host__ __device__ DualNumber atan2(const DualNumber& y, const DualNumber& x) {
    ...
    return DualNumber(atan2f(y.real, x.real), (x.real * y.dual - y.real *
x.dual) / denom);
}

template<typename T>
__host__ __device__ T pow(const T& base, double exponent) {
    return T(powf(base.real, exponent), exponent * powf(base.real, exponent - 1)
* base.dual);
}
...
namespace util {
  ...
  dual::DualNumber rosenbrock(const dual::DualNumber* x) {
      ...
      for (int i = 0; i < DIM - 1; ++i) {
          sum += dual::pow(1 - x[i],2) + 100 * dual::pow(x[i+1] -
  dual::pow(x[i], 2), 2);
          //sum = sum + 100 * (x[i + 1] - x[i] * x[i]) * (x[i + 1] - x[i] *
  x[i]) + (1 - x[i]) * (1 - x[i]);
      }
      return sum;
  }
  ...
  double rosenbrock(const double* x) {
      double sum = 0.0;
      for (int i = 0; i < DIM - 1; ++i) {
          sum += 100 * (x[i + 1] - x[i] * x[i]) * (x[i + 1] - x[i] * x[i]) + (1
  - x[i]) * (1 - x[i]);
      }
      return sum;
  }
  ...
  */
  ...
  dual::DualNumber ackley(const dual::DualNumber* x) {
      ...
      dual::DualNumber term1 = dual::DualNumber(-20.0) * dual::exp(-0.2 *
  dual::sqrt(sum_sq / DIM)); dual::DualNumber term2 = dual::DualNumber(0.0) -
  dual::exp(sum_cos / DIM); return term1 + term2 + 20.0 + dual::exp(1.0);
  }
  ...
  __device__ void calculateGradientUsingAD(double *x, double *gradient) {
      ...
      for (int i = 0; i < DIM; ++i) { // // iterate through each dimension
  (vairbale) xDual[i] = dual::DualNumber(x[i], 0.0);
      }

      // calculate the partial derivative of  each dimension
      for (int i = 0; i < DIM; ++i) {
          xDual[i].dual = 1.0; // derivative w.r.t. dimension i
          dual::DualNumber result = Function::evaluate(xDual); // evaluate the
  function using AD gradient[i] = result.dual; // store derivative
          //printf("\nxDual[%d]: %f, grad[%d]: %f
  ",i,xDual[i].real,i,gradient[i]); xDual[i].dual = 0.0;
      }
  }*/
...
dump_data_2_file(const Result<DIM>* h_results,
                 ...
                 const int PSO_ITER)
{
  std::string filename = "./data/" + fun_name + "_" + std::to_string(PSO_ITER) +
                         "psoit_" + std::to_string(DIM) + "d_particledata.tsv";
  ...
  std::cout << "\ndumped data 2 " << filename << "\n"
            << countConverged << " converged, " << stopped << " stopped early, "
                     ...
                     const double norm)
{
  ...
  if (file_empty) {
    outfile
      << "fun\tN\tidx\tstatus\tbfgs_iter\tpso_iter\ttime\terror\tfval\tnorm\t";
    for (int i = 0; i < dim; i++)
      outfile << "\tcoord_" << i;
    outfile << std::endl;
  } // end if file is empty
  ...
  outfile << fun_name << "\t" << N << "\t" << idx << "\t" << status << "\t"
          << max_iter << "\t" << pso_iter << "\t" << time_seconds << "\t"
...
launch_reduction(int N, double* deviceResults, Result<DIM>* h_results)
{
  ...
  // cudaMemcpy(h_results,d_results,N *
  // sizeof(Result<DIM>),cudaMemcpyDeviceToHost);
     ...
     double tolerance)
{
  ...
  if (PSO_ITER > 0) {
    pso_results_device =
      launch_pso<Function, DIM>(PSO_ITER, N, lower, upper, ms_init, ms_pso);
