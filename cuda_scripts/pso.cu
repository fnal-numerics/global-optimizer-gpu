...
void
hostPSOInit(FuncEval FunctionEval, // e.g. [&](const double* x){ return
                                   // Function::evaluate(x); }
            double lower,
            double upper,
            ...
            int PSO_ITERS = 10)
{
  ...
  auto randR = [&](double lo, double hi) { return lo + (hi - lo) * rand01(); };
                     ...
                     std::string fun_name)
{
  ...
  hostPSOInit([&](double const* x) { return Function::evaluate(x); },
              lower,
              upper,
              hostPSO,
              N,
              DIM,
              10); // last is iteration for PSO
  double* devicePSO = nullptr;
  cudaMalloc(&devicePSO, N * DIM * sizeof(double));
  cudaMemcpy(
    devicePSO, hostPSO, N * DIM * sizeof(double), cudaMemcpyHostToDevice);
  ...
  // lower, upper, deviceResults, deviceIndices, deviceCoordinates,
  // deviceTrajectory, N, MAX_ITER);
  if (save_trajectories) { // only allocate large trajectory array if user wants
                           // to!!
    cudaMalloc(&deviceTrajectory, N * MAX_ITER * DIM * sizeof(double));
    optimizeKernel<Function, DIM, 256>
  ...
  */
  ...
  // Append results to a .tsv file in precision 17
  {
    ...
    outfile << fun_name << "\t" << N << "\t" << MAX_ITER << "\t" << std::fixed
            << std::setprecision(17) << time_seconds << "\t" << error << "\t"
            << globalMin << "\t";
    for (int i = 0; i < DIM; i++) {
                         ...
                         int MAX_ITER)
{
  ...
  switch (choice) {
    ...
    case 7:
      ...
      // provide a path to a .cu file and compile it at runtime.
...
main(int argc, char* argv[])
{
  ...
  if (argc != 5) {
    std::cerr
      << "Usage: " << argv[0]
      << " <lower_bound> <upper_bound> <max_iter> <number_of_optimizations\n";
    return 1;
  }
  ...
  */
  ...
  while (cont == 'y' || cont == 'Y') {
    ...
    selectAndRunOptimization<dim>(
      lower, upper, hostResults, hostIndices, hostCoordinates, N, MAX_ITER);
