...
himmelblau_function <- function(x, y) {
  (x^2 + y - 11)^2 + (x + y^2 - 7)^2
}
grid$z <- himmelblau_function(grid$x, grid$y)

# Global minima of Himmelblau function
global_minima <- data.frame(
...
p <- ggplot() +
  ...
  labs(
    title = "Step-by-Step Optimization Trajectories on Himmelblau Function",
    x = "X-axis",
    y = "Y-axis"
  ) +
  theme_minimal() +
