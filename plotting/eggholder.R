...
# Eggholder grid parameters and function
x_min <- -512; x_max <- 512; y_min <- -512; y_max <- 512; num_points <- 500
x_seq <- seq(x_min, x_max, length.out = num_points)
y_seq <- seq(y_min, y_max, length.out = num_points)
grid <- expand.grid(x = x_seq, y = y_seq)
eggholder_function <- function(x, y) {
  - (y + 47) * sin(sqrt(abs(x/2 + y + 47))) - x * sin(sqrt(abs(x - (y + 47))))
}
grid$z <- eggholder_function(grid$x, grid$y)

# Global minimum for Eggholder is approximately at (512, 404.2319)
global_minima <- data.frame(x = 512, y = 404.2319)
...
p <- ggplot() +
  ...
  labs(title = "Optimization Trajectories on Eggholder Function",
       x = "X-axis", y = "Y-axis") +
  theme_minimal() +
  coord_fixed() +
...
print(p)
ggsave("optimization_trajectories_eggholder.pdf", plot = p, width = 8, height = 6, dpi = 300, device = cairo_pdf)
cat("Plot saved to optimization_trajectories_eggholder.pdf\n")
